...
def rr2l(name):
    ...
    with open(os.path.join(fasta_dir, name+".fasta")) as g:
        fasta = g.readlines()
        fasta = fasta[1].replace("U", "-").replace("B", "-").replace(
            "Z", "-").replace("X", "-").replace("O", "-").replace("\n", "")
...
def rrall(name):
    ...
    with open(os.path.join(rrall_dir, name)+".rr", "w") as g:
        ...
        g.write(
            "METHOD which can weight the sequences in MSA at the same time by the same network.\n")
        g.write(
            "METHOD Predicted secondary structure and others are also used as input features.\n")
        g.write("MODEL  1\n")
        # write fasata
